...
export const RequestMetadataRegex: RegExp =
  /^\s*(?:#|\/{2})\s*@([\w-]+)(?:\s+(.*?))?\s*$/;
//RequestMetadataRegex에 대한 설명 -> https://regexr.com/5h9qj
//^\s* : 공백으로 시작하는 문자열
//(?:#|\/{2}) : # 또는 //로 시작하는 문자열
//\s*@ : @로 시작하는 문자열
//([\w-]+) : 문자와 -로 이루어진 문자열
//\s+(.*?) : 공백이 있거나 없거나, 모든 문자열
//\s*$ : 공백으로 끝나는 문자열
//예시: @name value

export const CommentIdentifiersRegex: RegExp = /^\s*(#|\/{2})/;
//CommentIdentifiersRegex에 대한 설명 -> https://regexr.com/5h9qk
//^\s* : 공백으로 시작하는 문자열
//(#|\/{2}) : # 또는 //로 시작하는 문자열
//예시: #comment, //comment
...
export const FileVariableDefinitionRegex: RegExp =
  /^\s*@([^\s=]+)\s*=\s*(.*?)\s*$/;
//FileVariableDefinitionRegex에 대한 설명 -> https://regexr.com/5h9qf
//^\s*@ : @로 시작하는 문자열
//([^\s=]+) : 공백과 =이 아닌 문자열
//\s*=\s* : = 앞뒤로 공백이 있거나 없거나
//(.*?) : 모든 문자열
...
//LineSplitterRegex에 대한 설명 -> https://regexr.com/5h9qo
//\r?\n : \r이 있거나 없거나, \n이 있거나 없거나
//g : 전역 검색
//예시: "Hello\r\nWorld".split(LineSplitterRegex) -> ["Hello", "World"]
