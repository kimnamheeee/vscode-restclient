...
export class VariableProcessor {
  //VariableProcessor 클래스
  private static readonly providers: [HttpVariableProvider, boolean][] = [
    //providers를 선언하는데, HttpVariableProvider와 boolean을 원소로 갖는 배열로 초기화
  ...
  public static async getAllVariablesDefinitions(
    ...
    const [, [requestProvider], [fileProvider], [environmentProvider]] =
      this.providers;
    const requestVariables = await (
      requestProvider as RequestVariableProvider
    ).getAll(document);
