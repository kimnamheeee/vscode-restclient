...
export class RequestSettings implements Partial<IRestClientSettings> {
  ...
  public constructor(metadatas: Map<RequestMetadata, string | undefined>) {
    //인스턴스화될 때 호출하는 함수
    //metadatas는 Map<RequestMetadata, string | undefined> 타입이다.
    if (metadatas.has(RequestMetadata.NoRedirect)) {
      //metadatas에 NoRedirect가 있으면
      this._followRedirect = false; //followRedirect를 false로 설정
    } else if (metadatas.has(RequestMetadata.NoCookieJar)) {
      //metadatas에 NoCookieJar가 있으면
      this._rememberCookiesForSubsequentRequests = false; //rememberCookiesForSubsequentRequests를 false로 설정
    }
  }
}
